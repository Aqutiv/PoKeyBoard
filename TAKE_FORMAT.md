...
- `tempo.keySignature` (sharps positive, flats negative, −7…7) and `tempo.keyMode` (`major | minor`) are **optional** and never audible: the key an imported score declares in its first `<key>` — its `<fifths>`, and its `<mode>` when that names major or minor. The signature decides how pitches are spelled; the mode is what a MIDI export declares. Absent means the score never said, and each is read from the pitches instead. Added the same way `tempo.changes` was.

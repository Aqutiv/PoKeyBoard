 ...
 * caller a way out: an abort that lands before the take is swapped — during
 * the download or the save flushed ahead of it — leaves the active take alone,
 * so walking away mid-open changes nothing.
 */
export async function openLibraryTrack(trackId: string, signal?: AbortSignal): Promise<boolean> {
  ...
  return activateTake(take, signal);
}

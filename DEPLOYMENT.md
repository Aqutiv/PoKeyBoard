...
- [ ] **Adding a new instrument** needs neither bump — its files live at new immutable URLs and nothing existing becomes dead, so bumping would only make users re-download samples they already have. Do check the `ExpirationPlugin` `maxEntries` headroom in `src/pwa/service-worker.ts`: it is 512 against 91 entries per grand pack — 316 with all four pianos (Salamander, Headroom, Steinway, Wurlitzer) downloaded — and LRU eviction is silent
- [ ] **A pack built from pinned sources** (`bitklavier-grand-v1`, pins in `scripts/lib/bitklavier-grand-v1.pins.json`) rebuilds only from the exact bytes it was made from: the build stops on any size or SHA-256 mismatch. Re-pinning (`--pin`) is a deliberate act — review the diff, and if the audio changed, it is a new `-vN` like any other change to a published pack
- [ ] **Any new grand pack version** needs its velocity calibration: run `npx vitest run --config vitest.tools.config.ts tests/tools/generateVelocityCalibration.ts` and commit the regenerated `src/audio/velocityCalibration.ts`. `velocityCalibration.test.ts` fails until you do; without it the pack would play by the old per-layer trims, stepping in level wherever its velocity layers meet
- [ ] **…and its tone calibration**, after that one (it reads the velocity table for its headroom check): run `npx vitest run --config vitest.tools.config.ts tests/tools/generateToneCalibration.ts` and commit the regenerated `src/audio/toneCalibration.ts`. `toneCalibration.test.ts` fails until you do; without it the pack would play with no tone ramps, stepping in brightness wherever its velocity layers meet

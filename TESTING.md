...
15. **MP3 export & share sheet:** render a take; share sheet opens from the button; WhatsApp appears only when installed; the received file plays; on Firefox desktop the MP3 downloads instead.
16. **Offline launch:** enable airplane mode after downloading a piano in Settings → Piano → installed app launches, full keyboard plays, takes list intact. Download only one piano and confirm deleting it leaves the other still marked available offline.
17. **Piano choice:** switch piano in Settings → the preview note and the keys sound different at the same volume; hold a key down _while_ switching → the note releases cleanly and no key sticks; the choice survives a relaunch.
...
20. **Update flow:** deploy a new build → "update available" appears in Settings and applies only on request, never during recording.

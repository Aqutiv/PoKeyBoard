...
3. **Render** via `OfflineTakeRenderer`: `OfflineAudioContext` (2ch/48kHz, take + 3 s tail), the same `PianoGraphFactory` graph, `SampleBank` buffers, and envelope constants as live playback; sustain pedal pre-applied to durations. Every sample is chosen before the render starts, but the voices are made as it goes: the render pauses (`suspend`) every 2 s to make those of the next few seconds, because an offline context works through every voice it holds on every 128-frame quantum, started or not — made all at once, the 11-minute Chopin Ballade took 22 minutes to render in desktop Chrome, and this way takes about 20 s. Each pause is also an exact measure of how far the render has got, so the dialog's "Rendering piano…" bar counts up from it; until the first pause, while any missing samples decode, the bar slides instead. Firefox cannot pause an offline render, so there every voice is made up front, as before, and the render's bar slides throughout. The piano plays at the default volume and without the live peak guard (see Level below); the optional metronome is a click track rather than a render — one accented click and one plain one, each rendered by the live metronome's own synthesis, and the time of every beat — so it costs a few kilobytes instead of another full-length channel, and never counts toward the piano's loudness.
4. **Master and encode** in `mp3Encoder.worker`: channel copies are **transferred** (no clones), `masterExport` sets the level and holds the peaks in place, LAME encodes in ~2 s chunks, and the finished buffer transfers back. The "Compressing audio…" bar is one bar for both: mastering fills its first 15% — about its share of the time — counting its steps against a plan of the steps its passes will take, and the encoder the rest, chunk by chunk; the worker reports whole percents only as they rise. Progress from an export that has been cancelled, or has finished, is dropped, since a cancelled render runs on to its end. Where the worker cannot run, the same mastering and encoder run on the main thread a slice at a time — about 25 ms of work, then a turn for the page — so a long take never freezes it and Cancel is taken between slices; the file comes out the same either way.
